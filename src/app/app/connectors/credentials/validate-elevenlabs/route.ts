...
import { validateElevenLabsCredential } from "@/lib/connector-control-repository";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await validateElevenLabsCredential({ actorRole: role });
    return redirectTo(request, { role, validated: "ElevenLabs", feedback: "elevenlabs_validated" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "ElevenLabs credential validation failed.";
    return redirectTo(request, { role, error: message, feedback: "elevenlabs_validation_error" });
  }
}
