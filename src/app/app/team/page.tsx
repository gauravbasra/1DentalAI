  ...
  return (
    <FoundationShell active="/app/team" roleKey={role.key}>
      <PageHeader
        ...
        body="Phase 1 defines dental roles, data-class scopes, hidden-by-default areas, and role-specific module visibility before product workflows are built."
      />
        ...
        <div className="rounded-[2rem] border border-neutral-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {selectedRole.scopes.map((scope) => (
              <div key={scope} className="rounded-2xl bg-neutral-50 p-4">
                ...
                <p className="mt-1 text-xs text-neutral-500">Allowed scope in Phase 1 foundation.</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 rounded-2xl bg-amber-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-amber-900">
              {selectedRole.hiddenByDefault.length ? selectedRole.hiddenByDefault.join(", ") : "Nothing hidden by default for the synthetic owner role."}
            </p>
          </div>

          <div className="mt-6">
            <p className="text-sm font-semibold text-neutral-950">Synthetic team members</p>
            <div className="mt-3 space-y-3">
              {matchingTeam.length ? matchingTeam.map((member) => (
                ...
                <div className="rounded-2xl border border-neutral-200 p-4 text-sm text-neutral-600">
                  No seeded team member uses this role yet. The role still exists for RBAC and future tenant setup.
                </div>
              )}
            </div>
          </div>
