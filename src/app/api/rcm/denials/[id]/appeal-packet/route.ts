...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const packet = await buildRcmPacketForApi({
      ...
      packetType: "DENIAL_APPEAL",
      sourceRecordId: id,
    });
    return NextResponse.json({ ok: true, data: packet });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Unable to build denial appeal packet." }, { status: 404 });
  }
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const body = (await request.json().catch(() => ({}))) as DenialPacketActionBody;
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: "Unsupported action. Use one of: build, submit." }, { status: 400 });
  } catch (error) {
    const status = typeof (error as { status?: unknown }).status === "number" ? Number((error as { status: number }).status) : 500;
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Denial appeal packet action failed." }, { status });
  }
}
