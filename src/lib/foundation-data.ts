...
export type DailyDashboard = {
  ...
  workAreas: Array<{
    title: string;
    system: string;
    status: "Open" | "Setup required" | "Approval locked";
    route: string;
    primaryWork: string;
    nextAction: string;
  }>;
...
export const dailyDashboards: Record<RoleKey, DailyDashboard> = {
  owner_dentist: {
    ...
    workAreas: [
      { title: "PMS schedule and production board", system: "Practice management system", status: "Setup required", route: "/app/pms/schedule?role=owner_dentist", primaryWork: "Inspect today/tomorrow schedule, production holes, provider columns, and same-day treatment capacity.", nextAction: "Connect live PMS schedule before production writeback." },
      ...
      { title: "Clinical approval desk", system: "Clinical AI and charting", status: "Approval locked", route: "/app/pms/patients?role=owner_dentist", primaryWork: "Review AI scribe policy, clinical writeback approvals, imaging evidence, and provider signoff exceptions.", nextAction: "Clinical writeback remains locked until provider approval rules are configured." },
    ...
    kpis: [
      ...
      { label: "Today scheduled production", value: "$142.2K", detail: "$17.8K below daily goal unless holes are filled by 11 AM", tone: "amber" },
      { label: "Collections yesterday", value: "91%", detail: "$12.4K patient/insurance balance still collectable from recent visits", tone: "amber" },
      { label: "Unscheduled treatment", value: "$58.6K", detail: "23 patients; seven are on today's or tomorrow's schedule", tone: "red" },
      { label: "Case acceptance", value: "64%", detail: "Dollar-weighted acceptance lags patient-count acceptance by 11 points", tone: "amber" },
      { label: "Hygiene reappointment", value: "81%", detail: "Boulder is 9 points under target; five hygiene patients leave today without future visits", tone: "amber" },
      { label: "Provider production/hour", value: "$612", detail: "Dr. Marcus Lee is below goal due to 7.25 open chair hours", tone: "cyan" },
      { label: "Revenue leakage", value: "$31.4K", detail: "Underpayments, payer delay, attachment risk, and write-off review", tone: "red" },
    ],
    workQueues: [
      {
        title: "Yesterday close",
        items: [
          { label: "Celebrate production win, inspect collection gap", detail: "Production beat goal, but $12.4K remains in patient/insurance balances from recent visits.", meta: "Collections 91%", priority: "Today" },
          { label: "Same-day treatment accepted", detail: "$3.1K was added from previously unscheduled treatment in yesterday's hygiene visits.", meta: "Case acceptance", priority: "Watch" },
          { label: "Follow-up attempts incomplete", detail: "Six promised treatment or balance follow-ups were not completed before close.", meta: "Accountability", priority: "Today" },
        ],
      },
      {
        title: "Today production risk",
        items: [
          { label: "Fill two production holes", detail: "Open chair time can be filled from unscheduled treatment or overdue hygiene recall before 11 AM.", meta: "$7.8K capacity", priority: "Today" },
          { label: "Protect high-value unconfirmed visits", detail: "Four unconfirmed appointments carry $13.6K scheduled production and need direct outreach.", meta: "Confirmations", priority: "Today" },
          { label: "Provider schedule imbalance", detail: "Dr. Marcus Lee has open chair hours while emergency demand is active.", meta: "Production/hour", priority: "Today" },
        ],
      },
      {
        title: "Tomorrow opportunity",
        items: [
          { label: "Patients with treatment already scheduled for hygiene", detail: "Seven patients on tomorrow's hygiene schedule have open treatment plans worth $18.4K.", meta: "Morning huddle", priority: "Next" },
          { label: "Family members unscheduled", detail: "Five family members attached to tomorrow's patients are overdue for recall or restorative visits.", meta: "Retention", priority: "Next" },
          { label: "Membership plan candidates", detail: "14 uninsured active patients match practice plan criteria and need treatment coordinator follow-up.", meta: "$9.6K annualized", priority: "Watch" },
        ],
      },
      {
        title: "Revenue integrity and growth",
        items: [
          { label: "Approve revenue recovery worklist", detail: "12 claims show underpayment, missed attachment, or write-off patterns.", meta: "$18.9K recoverable", priority: "Today" },
          { label: "Payer delay by location", detail: "Aurora has five claims beyond expected payer response and one credentialing blocker.", meta: "RCM", priority: "Today" },
          { label: "Reputation recovery hold", detail: "Two negative feedback cases need owner-approved service recovery before review automation resumes.", meta: "Reviews", priority: "Watch" },
        ],
      },
    ],
    decisions: [
      "Assign owner for two schedule holes before 11 AM",
      "Approve payer recovery batch",
      "Escalate Aurora credentialing blocker",
      "Review provider production per hour with schedule mix",
      "Approve service recovery before review requests resume",
      "Set tomorrow huddle focus on open treatment in hygiene",
    ],
  ...
  associate_provider: {
    ...
    workAreas: [
      { title: "Patient chart worklist", system: "PMS/EHR chart", status: "Approval locked", route: "/app/pms/patients?role=associate_provider", primaryWork: "Open today’s charts, review medical alerts, treatment history, planned procedures, and unsigned clinical items.", nextAction: "Live chart writeback requires clinical approval setup." },
      { title: "AI scribe signoff", system: "Clinical AI and scribe", status: "Approval locked", route: "/app/pms/patients?role=associate_provider", primaryWork: "Review, edit, and sign AI-generated notes before any writeback.", nextAction: "Provider signoff workflow must be enabled." },
  ...
  rdh: {
    ...
    workAreas: [
      { title: "Perio charting board", system: "Charting and perio", status: "Approval locked", route: "/app/pms/patients?role=rdh", primaryWork: "Enter pocket depths, bleeding, mobility, recession, perio diagnosis support, and doctor-exam flags.", nextAction: "Perio writeback requires live chart model and provider rules." },
      { title: "Hygiene schedule", system: "PMS schedule", status: "Setup required", route: "/app/pms/schedule?role=rdh", primaryWork: "Work today’s hygiene column, exam timing, reappointment status, and recall opportunities.", nextAction: "Connect PMS schedule and recall data." },
  ...
  dental_assistant: {
    ...
    workAreas: [
      ...
      { title: "Procedure setup list", system: "PMS schedule", status: "Setup required", route: "/app/pms/schedule?role=dental_assistant", primaryWork: "See planned procedures, trays, materials, and provider-specific setup needs.", nextAction: "Connect live appointment procedure data." },
  ...
  front_desk: {
    ...
    workAreas: [
      ...
      { title: "Schedule confirmation board", system: "PMS schedule", status: "Setup required", route: "/app/pms/schedule?role=front_desk", primaryWork: "Confirm visits, fill openings, manage recalls, and move appointment requests into the schedule.", nextAction: "Connect live PMS schedule and reminder rules." },
      { title: "Forms and intake queue", system: "Forms", status: "Setup required", route: "/app/pms/schedule?role=front_desk", primaryWork: "Collect medical history, insurance cards, consents, and missing intake before arrival.", nextAction: "Enable forms storage and patient matching." },
  ...
  treatment_coordinator: {
    ...
    workAreas: [
      { title: "Treatment plan worklist", system: "PMS treatment planning", status: "Setup required", route: "/app/pms/treatment-plans?role=treatment_coordinator", primaryWork: "Review presented treatment, accepted-not-scheduled cases, doctor narratives, and case value.", nextAction: "Connect treatment plan and provider note data." },
      { title: "Financing and membership desk", system: "Patient financials", status: "Setup required", route: "/app/pms/treatment-plans?role=treatment_coordinator", primaryWork: "Offer financing, membership plans, payment schedules, agreements, and follow-up.", nextAction: "Connect financing, membership, and payment providers." },
  ...
  practice_manager: {
    ...
    workAreas: [
      { title: "Daily schedule control", system: "PMS schedule", status: "Setup required", route: "/app/pms/schedule?role=practice_manager", primaryWork: "Monitor schedule gaps, provider columns, confirmations, procedure mix, and production at risk.", nextAction: "Connect live schedule and production data." },
