...
| Appointment detail / checkout | Patient popout, appointment procedures, readiness, forms, benefits, copay, payment collection, completed procedures, next appointment, claims. Dentrix creates claims during checkout. | Appointment control page now links from the schedule, stages appointment procedures, evaluates forms/labs/imaging/insurance blockers, completes selected procedures into procedure logs, posts ledger charges, records patient payments, creates only internal READY claim drafts, and persists `PmsCheckoutSession` audit context. | Still needs next-visit scheduling, richer clinical signoff, payment allocation, claim attachment rules, and post-visit engagement triggers. | Appointment detail must be the visit control surface: arrive, seat, complete, checkout, collect, schedule next, create claim, trigger post-visit engagement. |
